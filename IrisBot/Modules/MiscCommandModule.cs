...
namespace IrisBot.Modules
{
    public class MiscCommandModule : InteractionModuleBase<ShardedInteractionContext>
    {
        ...
        public async Task HelpAsync()
        {
            ...
            sb.AppendLine("`/몬라조합 <몹이름>` : 해당 이름을 가진 몹의 조합법 및 정보를 출력합니다.");
            sb.AppendLine("`/몬라상자 <상자종류>` : 몬스터라이프 상점 상자에서 나오는 스페셜 몹과 확률을 출력합니다.");
            sb.AppendLine("`/몬라능력 <능력치 카테고리>` : 카테고리별로 스페셜 몹의 정보를 요약하여 출력합니다.");
            sb.AppendLine("`/신용점수 <닉네임>` : 레벨, 유니온, 길드, 활동량 등 객관적인 정보를 바탕으로 신용점수를 출력합니다.");
            eb.AddField("메이플스토리 (For KR servers)", sb.ToString());
